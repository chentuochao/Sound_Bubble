...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, input_dir, n_mics=6, sr=48000,
                dis_threshold = 1, directional=True, fair_compare = False,
                 prob_neg = 0, downsample = 1, mic_config = [], sig_len = 4.5, scale=False):
        super().__init__()
    ...
    def get_mixture_and_gt(self, curr_dir):
        ...
        if self.scale:
            scale = torch.abs(mixture).max()
            target_voice_inside /= scale
            mixture /= scale
