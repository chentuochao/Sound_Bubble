...
def generate_white_noise(noise_shape, max_level):
    ...
    white_noise = white_noise_level*torch.from_numpy(np.random.normal(0, 1, size=noise_shape)).float()
...
class WhitePinkBrownPerturbation:
    ...
    def __call__(self, audio_data, gt_audio):
        wn = generate_white_noise(audio_data.shape, self.max_white_level)
        pn = generate_pink_noise(audio_data.shape, self.max_pink_level)
        bn = generate_brown_noise(audio_data.shape, self.max_brown_level)
        # print("ssss")
        perturbed_audio = audio_data + (wn + pn + bn)

        return perturbed_audio, gt_audio
